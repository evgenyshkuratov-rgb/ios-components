...
- **icons-library**: https://github.com/evgenyshkuratov-rgb/icons-library - Icon assets (119 icons, 157 color tokens), Figma sync every 6h via GitHub Actions. Has its own MCP server (`mcp-server/`) with tools: `list_icons`, `list_colors`, `get_icon`, `check_updates`. Both MCP servers are registered globally in `~/.claude.json`. The GalleryApp build phase reads from this repo's local clone to populate status badge counts.
